...
function onTick()
    -- 1. 座標・オイラー角の取得
    if input.getNumber(24) ~= 0 then
        targetCoords = { input.getNumber(24), input.getNumber(25), input.getNumber(26) }
    end
    local ownCoords             = { input.getNumber(27), input.getNumber(28), input.getNumber(29) }
    local ownOrientation        = eulerZYX_to_quaternion(input.getNumber(32),input.getNumber(31),input.getNumber(30))
    local isLaunch              = input.getNumber(23) == 1

    local finStrength           = FIN_STRENGTH
    ...
    output.setNumber(1, yawAngle * finStrength)
    output.setNumber(2, pitchAngle * finStrength)
end
