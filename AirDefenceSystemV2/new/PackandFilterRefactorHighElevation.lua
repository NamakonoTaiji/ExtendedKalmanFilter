...
local PI = M.pi
local PI2 = PI * 2
local MAX_TARGETS_PER_RADAR = 8                       -- レーダーが同時に扱える最大目標数 (仕様 [source: 5])
...
function onTick()
    ...
    for i = 1, MAX_TARGETS_PER_RADAR do
        if inputBool(i) then -- 目標i を検出中か？
            ...
            targetMaxData[i].azimuth = M.max(correctedAzimuth, targetMaxData[i].azimuth)
            targetMaxData[i].elevation = M.max(correctedElevation, targetMaxData[i].elevation)

            -- 最小値更新
            targetMinData[i].distance = M.min(currentDistance, targetMinData[i].distance)
            targetMinData[i].azimuth = M.min(correctedAzimuth, targetMinData[i].azimuth)
            targetMinData[i].elevation = M.min(correctedElevation, targetMinData[i].elevation)
            -- else
