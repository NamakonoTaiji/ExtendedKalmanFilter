...
function onTick()
    ...
    if isLaunch then
        ...
        local local_azim_rad, local_elev_rad, local_elev_turn, local_azim_turn = 0, 0, 0, 0
        ...
        if guidanceStart then
            ...
            if launchTickCounter < INITIAL_GUIDANCE_TICKS then
                ...
                targetLocal = globalToLocalCoords(currentTargetPosVec, { x = ownPosX, y = ownPosY, z = ownPosZ },
                    ownOrientation)
                -- シーカーの指向方向を指示するためにローカル角度を計算
                local_elev_rad = math.asin(targetLocal.y / distance)
                local_azim_rad = math.atan(targetLocal.x, targetLocal.z)
            ...
            -- 回転基準に直す
            local_elev_turn = local_elev_rad / (math.pi * 2)
            local_azim_turn = local_azim_rad / (math.pi * 2)
            output.setNumber(3, local_azim_turn) -- ch3: シーカーX軸指向方向
            output.setNumber(4, local_elev_turn) -- ch4: シーカーY軸指向方向
        end
    end
end
