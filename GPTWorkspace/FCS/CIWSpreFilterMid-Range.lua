...
-- num4-6: レーダー目標2 ...
-- num19-21: レーダー目標7
-- num22-24: 火器管制から送られる迎撃対象座標
-- num32: 火器管制から送られる迎撃対象ID
...
-- num3: 仰角(ラジアン)
-- これを最大7目標分21chまで繰り返す
-- num22-24: 火器管制から送られる迎撃対象座標パススルー
-- num25: 自機ワールド座標X
-- num26: 自機ワールド座標Y
-- num27: 自機ワールド座標Z
-- num28: 自機オイラー角X
...
local ownOrientation = { 0, 0, 0 }
local ownGlobalPos = { 0, 0, 0 }
--------------------------------------------------------------------------------
-- ヘルパー関数 (ベクトル演算)
--------------------------------------------------------------------------------
...
function onTick()
    ...
    local sweepTime = input.getNumber(31) -- 経過時間
    ...
    if isRadarActive and sweepTime == (DETECTION_INTERVAL - 1) then
        ...
        for i = 1, #clusteredTargets do
            if i >= DETECTION_MAX then break end
    ...
    output.setNumber(22, input.getNumber(22))
    output.setNumber(23, input.getNumber(23))
    output.setNumber(24, input.getNumber(24))
