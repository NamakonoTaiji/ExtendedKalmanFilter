...
THREAT_HITS_THRESHOLD = property.getNumber("THREAT_HITS_THRESHOLD") or 5 -- 速度・指向条件を満たした同定成功の必要回数
...
function onTick()
    ...
    if isDetected or antiShipTargetInfos then -- 防空システムが目標を検出中もしくは対水上モードでの発射キューがある場合
        ...
        if isDetected and isThreat and detectedTarget.detectionTickLag == 1 and threatHits >= THREAT_HITS_THRESHOLD and checkThreatTargetsID then
            -- このターゲットは迎撃すべき脅威と判定
