...
function onTick()
    ...
    isTargetTrackMode = input.getBool(30)   -- ターゲット選定有効化フラグ。これがTrueの場合にレーダー視点の中心距離に一番近い目標のトラッキングを開始する。

    radarManualSweepX = input.getNumber(31) -- 座席の視点X(azimuth/turn)
    radarManualSweepY = input.getNumber(32) -- 座席の視点Y(elevation/turn)
    ...
    -- (E) プライマリターゲットの選択 (視線に最も近いトラック)
    local primaryTrackID = nil
    local minAngleDiffSq = math.huge

    -- パイロット視線のグローバル角度 (仮の計算、要調整)
    --  pilotGlobalAzimuth = ownEuler.Yaw + radarManualSweepX * PI2 -- 要確認
    --  pilotGlobalElevation = ownEuler.Pitch + radarManualSweepY * PI2 -- 要確認
    ...
    for trackID, track in pairs(trackedTargets) do
        ...
        -- トラック位置を自機ローカル座標に変換 (角度比較のため)
        local trackLocalPos = globalToLocalCoords(trackGlobalPos, ownGlobalPos, ownOrientation)

        local angles = localCoordsToLocalAngle(trackLocalPos)
        -- パイロット視線角度 (ラジアン)
        local pilotViewAzimuthRad = radarManualSweepX * PI2
        local pilotViewElevationRad = radarManualSweepY * PI2
        -- 角度差の2乗を計算 (単純な差で近似)
        local angleDiffSq = (calculateAngleDifference(angles.azimuth, pilotViewAzimuthRad)) ^ 2 +
            (calculateAngleDifference(angles.elevation, pilotViewElevationRad)) ^ 2

        if angleDiffSq < minAngleDiffSq then
            minAngleDiffSq = angleDiffSq
            primaryTrackID = trackID
        end
    end

    -- 目標ロック時はprimaryTrackIDを固定させる
    if isTargetTrackMode and trackedTargets[trackingID] then
        primaryTrackID = trackingID
    else
        trackingID = primaryTrackID
    end
