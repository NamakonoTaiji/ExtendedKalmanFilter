...
function onTick()
    ...
    ------ 追跡中ターゲットをローカル角度に変換
    if trackingTarget.x ~= 0 or trackingTarget.z ~= 0 then
        ...
        local targetLocalCoords = globalToLocal(
            { x = trackingTarget.x, y = trackingTarget.y, z = trackingTarget.z },
        ...
        local waypoint_1_LocalAngle = localCoordsToLocalAngle(targetLocalCoords)

        table.insert(targetInfoFromCam, {
            dist = distance,
            localCoords = targetLocalCoords,
            isWaypoint = true,
            drawTextString = ""
        })
    end
...
function onDraw()
    if seatViewX ~= 0 and seatViewY ~= 0 then -- Scopeを使っていないときのみHMDを描画
        ...
        for i, target in ipairs(targetInfoFromCam) do
            ...
            if rel_z > 0.1 then
                ...
                -- 10. 描画
                screen.drawCircle(screenX, screenY, 1)
                ...
                if trackingID ~= 0 then
                    screen.drawText(screenX, screenY - 9, "ID " .. math.floor(trackingID))
                    screen.drawText(6, 10, string.format("%.5f", epsilon))
                    screen.setColor(255, 255, 0, RGB_ALPHA / 2)
                    screen.drawCircle(screenX, screenY, epsilon * 10)
                end
            end
        end
    end
