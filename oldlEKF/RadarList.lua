...
function onTick()
    filterDist = input.getNumber(32) * maxRange -- 味方艦が存在するときはmaxRangeを狭めることでシステムを動作させないようにする
    maxOutNumber = 12
    currentSetedNumber = 0
    for i = 1, 12 do
    ...
    for i = 1, 6 do
        local a, b, dist
        a, b = input.getNumber(i * 2 - 1), input.getNumber(i * 2)
        if a * b == 0 then
            dist = 0
        else
            -- 距離を抽出
        ...
        if input.getNumber(i * 2 - 1) ~= 0 and dist < filterDist then
            table.insert(PackA, { p1 = input.getNumber(i * 2 - 1), p2 = input.getNumber(i * 2) })
        end
    end
    ...
    for i = 7, 12 do
        ...
        if input.getNumber(i * 2 - 1) ~= 0 and dist < filterDist then
            table.insert(PackC, { p1 = input.getNumber(i * 2 - 1), p2 = input.getNumber(i * 2) })
        end
    end
